      ...
      <div class="row">
        <div class="col-12">
          @component('component.table',['istfoot'=>0,'table_class'=>'text-center'])
            @slot('thead')
              <tr>
                <th>Approvel</th><th>Status</th><th>Remark</th>
              </tr>
            @endslot
            @forelse ($data->load('Approve_List')->Approve_List as $value)
              <tr>
                <td>{{Adldap::search()->where('sAMAccountName',$value->user_id)->get()[0]->cn[0]}}</td>
                <td>
                  @if ($value->status==2)
